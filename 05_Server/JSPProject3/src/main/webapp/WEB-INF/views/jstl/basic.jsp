...
<body>
  ...
  <c:set var = "num2" value = "12345" scope = "session"/>
  <h4>session scope num2 : ${sessionScope.num2}</h4>

  <hr>
  <c:set var = "num3" value = "3333" scope = "application"/>
  <h4>application scope num3 : ${applicationScope.num3}</h4>


  <hr>
  <h1>2. 변수 제거(==속성제거) (c:remove태그)</h1>
  ...
  HTML 태그 해석 X : <c:out value = "${temp}" escapeXml = "ture"/>
  <br> 
  HTML 태그 해석 O : <c:out value = "${temp}" escapeXml = "false"/>

  <br>
  EL : EL로 출력시 escapeXml = "false" 상태 적용 (HTML해석함)
   ${temp}
  <hr>
</body>
</html>