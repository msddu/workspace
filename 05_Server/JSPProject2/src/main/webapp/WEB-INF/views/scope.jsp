...
<body>
  ...
  <pre>
    ...
    <h4>3. session (입회, 접속)</h4>
      -session : 서버에 접속한 클라이언트를 나타내거나,
                 관련 정보를 get/set 할 수 있는 객체(session객체는 서버에서 관리)
    ...
    <h4>5. 내장 객체의 우선순위 : page > request > session > application   (좁을수록 먼저)</h4>
  </pre>
  ...
  <% //page scope
    pageContext.setAttribute("str", "page scope에 세팅된문자열");
  %>
