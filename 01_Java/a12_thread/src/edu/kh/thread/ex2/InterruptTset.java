...
public class InterruptTset {
	 ...
	 * 
	 * void interrupt()
	 * -현재 스레드 interrupted필드 값을 true로 변경
	 *  interrupted 필드 == false  -> 작업 진행
	 *  interrupted 필드 == true  -> 작업 멈춤
	 * 
	 *  단, Thread.sleep() 에 의해서 일시 정지된 스레드에 
	 *  interrupt()를 호출하게 되면
	 *  스레드의 interrupted필드 값이 true로 변하지 않고(작업종료x)
	 *  InterruptedException이 발생한다.
	 * 
	 * 
	 * boolean isInterrupted()
	 * -현재 스레드의 interrupted필드 값을 반환(getter)
	 * 
	 * 
	 * 
	 * booleanThread.interrupted()
	 * -현재 스레드의 interrupted필드 값을 반환 + false로 값을 변경
	 * 
	 * */
