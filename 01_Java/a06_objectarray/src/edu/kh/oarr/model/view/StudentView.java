...
public class StudentView {
	...
	private void selectAll() {
		...
		for(Student s : arr) {
			...
			if(s == null) {//참조하는 학생 객체가 없는 참조 변수일 경우
				break;
			}
	...
	private void selectIndex () {
		...
		//그런데 index에 학생이 없거나 범위가 초과되면 null을 반환
		
		Student s = service.selectIndex(index);
		
		if( s== null) {//범위 초과 또는 학생없음
	...
	public void selectScore() {
		...
		//인덱스를 입력받아 서비스로 전달하여
		//반환 받은 문자열을 출력
		System.out.print("조회할 학생 인덱스 입력하세요: ");
		int index = sc.nextInt();
	...
	public void updateStudent() {
		...
		//인덱스 번호를 입력받고
		//해당 학생이 존재하면 국어,영어,수학 점수를 입력 받아 수정 수 
		//"수정 되었습니다" 출력
		
		//존재하지 않으면 
		//"해당 인덱스에 학생 정보가 존재하지 않습니다."출력
		
		//1. 인덱스 번호를 입력받아 service로 전달하여 
	...
	public void selectName() {
		...
		Student[] arr = service.selectName(name);  //이름이 일치하는 모든 학생을 조회하는 서비스 호출
