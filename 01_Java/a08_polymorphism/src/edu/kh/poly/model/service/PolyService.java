	...
	public void printCar() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			System.out.println(arr[i].toString());
			 // Car자료형(클래스)의 toStirng()으로만 연결
			 //-> 왜 arr[i]의 자료형이 Car라서!
			 //-->정적 바인딩
			
			//bind : 묶다, 연결하다
			//바인딩 ㅣ 메서드 호출부와 실제 실행되는 메서드 코드를 묶는 것
			
			//정적 바인딩
			//-> 프로그램 실행 전(컴파일 단계) 메서드 호출부와 메서드 코드를 묶는 것
			//--> 참조변수의 자료형을 토대로 묶임
			
			//동적 바인딩
			//-> 프로그램 실행 중 메서드 호출부와 참초 변수가 가리키는 실제 객체 타입의 오버라이딩된 메서드와 연결이 되는 것
			
			//*업캐스팅(부모 참조 = 자식 객체) 시 호출되는 메서드는 오버라이딩한 자식 객체가 우선권을 갖는다.
		}
		...
		//*동적 바인딩은 컴파일러로 검출이 안되기 때문에 실행 결과가 예상과 다르면 모두 개발자의 실수가 되어버림...
		//->다형성이 적용된 코드를 이용할 땐
