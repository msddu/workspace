...
function classTest(){
  ...
  //클래스 속성값이 div2인 객체 모두 얻어와 배열로 반환받기
...
function tagTest(){
  ...
  for(let i = 0; i<arr.length; i++){
    arr[i].style.backgroundColor = "rgb(220,140,"+arr[i].innerText+")"
  }
}
...
// CSS선택자로 접근하기
function cssTest(){
  //querySelector("선택자") : 1개만 얻어옴 (여러요소 있으면 첫번째)
...
function readValue(){
  ...
  //"문자열".length : 문자열의 길이
  //"문자열".trim() : 양쪽 공백 제거  
  // 입력요소 값.trim().length == 0 : 아무것도 입력되지 않았다면
  if(input.value.trim().length==0) { /*  아무것도 입력하지 않았다면 */
    alert("채팅 내용을 입력해주세요");
  }else{  /* 입력했을때 */
    bg.innerHTML += "<p><span>"+input.value+"</span></p>";

    // 요소.scrolHeight : 스크롤 전체 높이
    // 요소.scrollTop : 요소 내부 스크롤의 위치 반환 
    // 요소.scrollTop = 높이 : 스크롤의 위치를 해당 높이로 이동
