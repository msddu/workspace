...
public class MyPageServiceImpl implements MyPageService{
	...
	//정상 수행 시 commint, 모든예외 발생 시 rollback
	...
	public int info(Member updateMember, String[] memberAddress) {	
		...
		//1) memberAddress 가공 처리
	      ...
	      //2) DAO호출 후 결과를 반환 받아 바로 Controller로 반환
	...
	public int changePw(String currentPw, String newPw, int memberNo) {
		...
		//   왜? 마이바티스 코드는 파라미터를 하나만 전달할 수 있어서!!!(우리가 전달해아할 파라미터는 2개)
		 Map<String, Object> map = new HashMap<String, Object>();
		 map.put("newPw", bcrypt.encode(newPw)); //새 비밀번호 암호화
		 map.put("memberNo", memberNo);
		 
		 //4. DAO 메서드 호출 후 반환된 결과를 
		 //   Controller로 반환 + @Transational(DML구문이기 때문에)
	...
	public int secession(String memberPw, int memberNo) {
		...
		//1. 입력받은 비밀번호가 DB에 저장된 로그인한 회원의 비밀번호와 같다면 회원탈퇴 수행 후 결과 반환
		String encPw = dao.selectMemberPw(memberNo);
