...
public class WebSocketConfig implements WebSocketConfigurer{
	...
	//웹소켓 핸들러 등록
	//-어떤 주소로 웹소켓 요청 왔을때 이를 처리하기 위한 웹소켓 핸들러 객체와 연결설정
	...
	public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
		...
		//setAllowedOriginPatterns : 웹소켓 요청 허용 주소 패턴을 작성 가능
		
		//withSockJS : SockJS 지원, 브라우저 호환성 증가
		
		//addInterceptors : 요청을 핸들러로 전달할 때 가로채기 역할을 할 객체를 추가
		
		registry.addHandler(testWebsocketHandler, "/testSock")
		.setAllowedOriginPatterns("http://localhost/","http://127.0.0.1/","http://192.168.10.121/")
		.withSockJS();
