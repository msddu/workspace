...
public class AjaxController {
	...
	public String SelectMemberNo(int memberNo) {
		...
		//**비동기 방식으로 응답하는 방법**
		//-비동기 통신은 페이지 갱신(화면 새로 만들기)이 목적x
		//-단순히 요청에 맞는 응답 데이터만 주고 받는 것이 목적o
		
		//조회 결과 email을 요청한 곳으로 반환
		return email; //
		
		// **문제 발생
		//-email을 반환 하려는데 controller는 반환된 값은 viewResolver로 전달해
		// 자동으로 /templates/반환값.html로forward를 수행
		
		//[해결방법]
		//controller 의 메서드 반환값이 forward를 위한 경로가아닌 
		//비동기 요청한 곳으로 돌아가야하는 값자체임을 명시하는 @ResponseBody 를 메서드 위에 추가
	}
	...
	//- HttpMessageConverter 가 이 과정에서 데이터 타입을 Java에 맞게 변환
	// number -> int/double
	// string -> String
	// JSON -> DTO,List,Map
	...
	@ResponseBody //반환 값이 그대로 돌아감(비동기통신)
