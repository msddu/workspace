...
@Component //bean등록(Spring container가 관리하는 객체)
@Slf4j
...
//	@Scheduled(fixedRate = 5000) //이전 작업 시작 후 5초 후에 수행
	
//	cron="초 분 시 일 월 요일 [년도]" - 요일 : 1(SUN) ~ 7(SAT)	
//	@Scheduled(cron = "0,30 * * * * *") //시간 초단위가 0,30인 경우 수행
	@Scheduled(cron = "0 0 * * * *") //시간 초단위가 0 0인 경우 수행 -> 정시마다 수행
//	@Scheduled(cron = "0 0 0 * * *") //시간 초단위가 0 0 0 인 경우 수행 -> 자정마다 수행
//	@Scheduled(cron = "0 0 12 * * *") //시간 초단위가 0 0 12 인 경우 수행 -> 정오마다 수행
//	@Scheduled(cron = "0 0 0 1 * *") //시간 초단위가 0 0 0 1 인 경우 수행 -> 매달 1일 자정 수행
//	@Scheduled(cron = "0,15,30,45 * * * * *") //시간 초단위가 0,15,30,45인 경우 수행
	public void schedeling() {
		log.info("스케쥴러 동작");
		
		//DB, 서버 파일 목록 비교 후 DB에 없는 서버 이미지 파일 삭제 동작
		
		//1. 서버 파일 목록 조회하기
		File boardFolder = new File(boardLocation);
		File memberFolder = new File(memberLocation);
		
		//2. 참조하는 폴더에 존재하는 파일 목록 얻어오기
		...
		//3. 서버, DB 이미지 파일명을 비교하여 서버에만 있고 DB에 없는 파일을 삭제
