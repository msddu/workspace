...
memberEmail.addEventListener("input" , ()=>{
  ...
  //   ^: 시작  
  //   $: 끝
  //   [A-Za-z\d\-\_] :  영어 대/소 + 숫자(0~9)+-_ 
  //   {4,}@ : 4글자 이상
  //   [가-힣\w\-\_] :  한글 + 영어 + 숫자
  //   +: 1글자 이상
  //   (\.\w+) : .으로 시작하는 아무 단어 1글자 이상
  //   {1,3} : 1개 이상 3개 이하
  ...
  if(regEx.test(memberEmail.value)){
    ...
    .then(result => {
      if(result == 0){  //중복x
        ...
        emailMessage.classList.add("confirm");  //초록색 글씨 추가
        emailMessage.classList.remove("error"); //빨간색 글씨 제거
        checkObj.memberEmail = true;   //유효한 상태임을 기록
      }
      else{  //중복o
        emailMessage.innerText = "이미 사용중인 이메일  입니다"
        emailMessage.classList.add("error");  //초록색 글씨 추가
        emailMessage.classList.remove("confirm"); //빨간색 글씨 제거
  ...
  //입력 받은 이메일이 정규식과 일치하지 않은 경우
    ...
    emailMessage.classList.remove("confirm");  //초록색 글씨 추가
    emailMessage.classList.add("error"); //빨간색 글씨 제거
    checkObj.memberEmail = false;   //유효하지 상태임을 기록
  }
  ...
  if(regEx.test(memberNickname.value)){
    ...
    .then(result => {
      if(result  == 0){
        ...
        nickMessage.innerText = "이미 사용중인 입니다."
...
document.getElementById("signUpFrm").addEventListener("submit", e=>{
  ...
  /* checkObj의 모든 값을 검사해서 하나라도 false이면 가입 시도X */

  //객체 전용 향상된 for문 : (for ... in)
  for(let key in checkObj){

    //객체에서 얻어온 값이 false인 경우(유효x)
    if(!checkObj[key]){

      let str;
      switch(key){
        case "memberEmail":      str = "이메일이 유효하지 않습니다"; break;
        case "authKey":          str = "인증번호가유효하지 않습니다"; break;
