...
boardLike.addEventListener("click", e=>{
  ...
  //1. 로그인 여부 확인(로그인한 회원만 좋아요 누를 수 있다!)

  // ***로그인 여부 확인 방법***
  //session에 loginMember가 있으면 로그인 상태 
  //                        없으면 로그아웃
  //-> session을 이용해야함 
  //   JS만으로는 server session 사용 불가능
  //   단, Thymeleaf를 작성할 수 있는 html 파일에서는 js와 혼용 가능

  //*html 파일에 boardDetail.js를 추가하기 전에 script를 이용해서 전역변수로 로그인 여부 저장
...
if(goToListBtn != null){ //화면에 목록으로 버튼이 있을 때만 수행
  ...
  const goToListFn = ()=>{
    ...
    //URL객체.searchParams : 주소에 담긴 쿼리스트링 map형식으로 반환

    const paramMap = new URL(location.href).searchParams;
    const obj = {}; //주소에 담겨있는 파라미터를 담은 객체

    //paramMap 데이터를 obj객체에 대입
    obj.cp = paramMap.get("cp");
    obj.key = paramMap.get("key");
    obj.query = paramMap.get("query");

    //상세페이지 주소 : /board/{boardCode}/{boardNo}?cp=1&key=t&query=text

    //목록 페이지 주소 : /board/{boardCode}?cp=1&key=t&query=text
