...
//-----------------------------------------------------------------------------
/*회원 조회 버튼 클릭 시 모든 회원정보를 조회*/
const btn5 = document.getElementById("btn5");
const result5 = document.getElementById("result5");

btn5.addEventListener("click", ()=>{

  //파라미터(쿼리스트링)이 없는 GET방식 비동기 요청
  ...
  .then(response => {
    ...
    if(response.ok){
      console.log("비동신 통신 성공");

      //json 형태의 응답 데이터(memberList)를
      //JS객체 형태로 변환해서 반환
      //(첫 번째 then에서 리턴 시 promiseㄱ개체에 데이터만 반환됨)
  ...
  .then(memberList => {
    ...
    for(let member of memberList){
      ...
      //탈퇴여부 'Y', 'N'을 속성으로 추가
      btn.setAttribute("date-flag", member.memberDelFl);

      //==================================================================================
      //버튼 클릭 시 회원 탈퇴 복구
      btn.addEventListener("click", e=>{

        //버튼의 속성 중 "data-falg"값이 'N'이면 'Y' , 'Y'이면 'N' 저장
        // -> DB UPDATE 시 MEMBER_DEL_FL 에 대입할 값으로 사용
