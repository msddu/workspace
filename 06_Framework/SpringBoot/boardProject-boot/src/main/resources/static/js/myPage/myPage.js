...
// - 파일이 선택화면에서 취소를 누르면 기존에 세팅된 value/files가 없어지게 된다.
//   -->1차 이미지 변경 후 2차 변경 과정에서 취소를 누르면 
//      1차 이미지 value/files가 없어지게 되어 제출 시 파일이 서버로 전달되지 않는다.

// - value에 대입 가능한 값은 빈칸("")뿐이다.

// - files는 동일한 자료형(FileList)또는 undifined 대입 가능
...
//input type = "file" 태그가 변경 되었을 때 변경된 상태를 백업해둘 변수
//요소.cloneNode(true,false) : 요소 복제(true 이면 하위 요소도 복제)
let backupInput; 
