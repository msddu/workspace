...
//x버튼이 클릭된 input 태그의 순서를 기록할 Set 생성
//(Set 특징 : 순서x, 중복허용x -> x버튼 여러번 눌러도 하나만 추가)
...
for(let i = 0; i<inputImageList.length; i++){
  ...
  deleteImageList[i].addEventListener("click",()=>{
    ...
    //미리보기 삭제
    previewList[i].removeAttribute("src");  //src속성을 제거(img태그)
...
boardUpdateFrm.addEventListener("submit", e=>{
  ...
  //hidden 타입 태그에 삭제한 이미지 번호 목록을 추가(1,2,3 형태)
