...
SELECT BOARD_NO,BOARD_TITLE,BOARD_CONTENT,BOARD_WRITER,BOARD_PW,WRITE_DATE,BOARD_DEL_FL
FROM "TB_BOARD"
...
WHERE BOARD_NO = 1495) C
      WHERE COMMENT_DEL_FL = 'N'
      START WITH PARENT_NO IS NULL
      CONNECT BY PRIOR COMMENT_NO = PARENT_NO
      ORDER SIBLINGS BY COMMENT_NO;

--START WITH PARENT_NO IS NULL : PERENT_NO가 없는 댓글이 계층의 시작지점이다
--CONNECT BY PRIOR COMMENT_NO = PARENT_NO : PERETN_NO, COMMENT_NO가 같은 행을 연
-- ORDER SIBLINGS BY COMMENT_NO : 계층 구조 정렬 순서
--LEVEL: 계층의 단계를 출력하는 가상 컬
