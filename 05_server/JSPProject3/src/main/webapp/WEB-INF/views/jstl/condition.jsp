...
<%-- format태그 : 형식,형변환 관련 코드(fmt태그 사용가능) --%>
...
<c:set var="menu" scope="page" value="순두부찌개"/>
...
<pre>
  -c:choose 태그 : c:when / c:otherwise 태그를 감싸는 태그
                  (이 안에 if ~ else if ~ else 구문을 쓰겠다)
    ***c:choose 태그 작성 주의사항***
    choose 태그 내부에는 
    c:when, c:otherwise, JSP주석, 공백 이외에는 아무것도 작성되면 안된다!!!
  ...
  -c:otherwise(그렇지 않다면)태그 : else역할의 태그로 아무런 속성이 없음!!               
