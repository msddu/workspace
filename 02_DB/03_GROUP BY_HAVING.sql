...
SELECT DEPT_CODE "묶인그룹의이름" , SUM(SALARY) "그룹별급여합"
FROM EMPLOYEE 
GROUP BY DEPT_CODE  --EMPLOYEE테이블에서 
...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE , COUNT(BONUS) /*보너스가 NULL인 행 제외하고 카운트*/
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
-- 부서별 급여 평균이  3,000,000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
...
--    EMP_NO LIKE '7%'
--    SUBSTR(EMP_NO,1,6) BETWEEN  '700101' AND '7912341'
GROUP BY DEPT_CODE
...
--부서 코드가 'D5' 또는 'D6'인 사원이름, 부서코드 조회
