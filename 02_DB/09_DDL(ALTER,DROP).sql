...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_COPY_ID_CHK CHECK(LOCATION_ID IN('L1', 'L2', 'L3', 'L4', 'L5') );
...
ALTER TABLE DEPT_COPY DROP 
CONSTRAINT DEPT_COPY_ID_CHK;



-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY 
DEPT_ID CONSTRAINT SYS_C0014075 NULL; --DEPT_ID NOT NULL 제거

ALTER TABLE DEPT_COPY MODIFY 
LOCATION_ID CONSTRAINT SYS_C0014076 NULL; --LOCATION_ID NOT NULL 제거

ALTER TABLE DEPT_COPY MODIFY 
DEPT_TITLE CONSTRAINT SYS_C0014113 NULL; --DEPT_TITLE NOT NULL 제거
...
/*DDL, DNL 중 작업 중인 SQL을 확실히 마무리 한 후 진행하기*/
 ...
 *2. "COMMENT" -> COMMENT 테이블 작성 시 " " 감싸기
 **/
...
COMMENT ON COLUMN MEMBER.MEMEBER_NO IS '회원번호';
...
COMMENT ON COLUMN BOARD.BOARD_TITLE IS '게시글목록';
...
COMMENT ON COLUMN "COMMENT".COMMENT_DEL_FL IS '댓글삭제여부(N:삭제X, Y:삭제O)';
COMMENT ON COLUMN "COMMENT".BOARD_NO IS '댓글이삭제된게시글번호';
COMMENT ON COLUMN "COMMENT".MEMBER_NO IS '댓글작성자회원번호';
