...
--EMPLOYEE테이블에서 
--사원명, 이메일, 이메일 길이 조회
--단, 이메일 길이가 12이하 인 행만 조회
--이메일 길이 오름차순으로 조회
SELECT EMP_NAME "사원 이름" , EMAIL "이메일" , LENGTH(EMAIL) "이메일길이"
FROM EMPLOYEE
...
--문자열에 검색을 5번부터 시작해서 처음 나오는 'B'의 위치를 조회
SELECT 'AABAACAABBAA' , INSTR('AABAACAABBAA', 'B', 5) 
FROM DUAL; --9

--문자열에 검색을 5번부터 시작해서 두번째 나오는 'B'의 위치를 조회
...
--이메일 아이디 오름차순으로 조회
SELECT EMP_NAME "사원 이름", SUBSTR(EMAIL , 1, INSTR(EMAIL , '@')-1) "아이디"
FROM EMPLOYEE
ORDER BY "아이디";
...
--SYSDATE : 현재 시간
--SYSTIMESTAMP : 현재 시간(ms포함, 표준 시간대)
...
--날짜 + 숫자 : 일 단위로 연산 (윤년 계산이 안됌)
--MONTHS_BETWEEN() : 윤년 계산이 알아서 포함되어 있음


--ADD MONTHS(날짜, 숫자): 날짜를 숫자만큼의 개월수를 더하여 반환
SELECT SYSDATE "오늘 날짜", ADD_MONTHS(SYSDATE , -1) "한달 전"	 , ADD_MONTHS(SYSDATE , 1) "한달 후"
FROM DUAL;
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" DAY PM HH24 "시"  MI "분" SS"초" ') FROM DUAL;
...
/**연도 패턴 Y, R 차이점***/
--연도가 두자리만 작성되어 있는 경우
--50미만 : Y,R 둘 다 누락된 연도 앞부분에 현재 세기(21C == 2000년대)추가
--50이상 : Y는 현재 세기(2000년대)추가
--        R은 이전 세기(1900년대)추가
...
--EMPLOYEE테이블에서
--사번, 이름 ,전화번호 조회
--단, 전화번호가 없으면(NULL)이면 '없음' 으로 조회

SELECT EMP_ID"사번", EMP_NAME "이름", NVL(PHONE, '없음') "전화번호"
FROM EMPLOYEE;

--EMPLOYEE테이블에서 
--이름, 급여, 보너스를 조회 
--보너스가 없으면 0 으로 조회
SELECT EMP_NAME "이름" , SALARY "급여", NVL(BONUS, 0) "보너스" 
FROM EMPLOYEE; 

--EMPLOYEE테이블에서
--사원의 이름, 연봉, 인센티브(연봉 * 보너스) 를 조회
--단, 인센티브가 없으면 0으로 표시
--연봉 내림차순 으로 조회
