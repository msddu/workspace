...
--DEPT_ID 중 일치하는 값이 없어서 합친(JOIN) 결과에 포함X
...
FROM DEPARTMENT 
JOIN LOCATION ON(LOCATION ID = LOCAL_CODE);
...
-->LEFT JOIN : JOIN구문 기준으로 
--             왼쪽에 작성된 테이블의 모든 행이
--             JOIN결과에 포함되게 하는 JOIN
...
--RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --RIGHT JOIN(24행)
...
SELECT E.EMP_NAME, E.SALARY, E.SAL_LEVEL, S.SEL_LEVEL
FROM EMPLOYEE E
...
--사번, 이름, 사수번호 ,사수이름 조회
--EMP_ID, EMP_NAME, MANAGER_ID, ENP_NAME
--하나의 테이블만 생각하지말고 똑같은 테이블이 두 개가 있다고 생각!
-- ANSI 표준
...
JOIN DEPARTMENT ON(DEPT_CODE  = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE SUBSTR(EMP_NO , 8, 1) = '2' AND SUBSTR(EMP_NO, 1 ,1) = '7' AND EMP_NAME LIKE '전%';

-- 2. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 직급명 을 조회하시오.
SELECT EMP_ID 사번, EMP_NAME 사원명, JOB_NAME 부서명
FROM EMPLOYEE 
NATURAL JOIN JOB 
...
SELECT EMP_NAME , BONUS , DEPT_ID , LOCAL_NAME 
FROM EMPLOYEE 
JOIN DEPARTMENT  ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
