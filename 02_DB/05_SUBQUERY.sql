...
/*  서브쿼리 유형
    ...
    - 단일행 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때   (행이 1개)
...
-- EMPLOYEE테이블의 DEPT_CODE와 동일한 사원을 구하시오. --> 한국부서에서 일하는 사원
...
--모든 사원의 사원명, 급여, 전체사원 급여 평균 조회			 
SELECT EMP_NAME, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE ) "전체사원급여평균"
FROM EMPLOYEE;
